...
class NaviControl():
  ...
  def get_navi_speed(self, sm, CS, cruiseState_speed):
    ...
    if not self.speedlimit_decel_off:
      ...
      elif self.sm['liveMapData'].speedLimit > 19 and self.osm_speedlimit_enabled and not self.sm['controlsState'].osmOffSpdLimit:  # osm speedlimit
  ...
  def auto_speed_control(self, CS, navi_speed, path_plan):
    ...
    #   CS.set_cruise_speed(ctrl_speed)
      ...
      if CS.out.brakeLights and CS.out.vEgo == 0:
        ...
        if self.cut_in_run_timer and dRel < CS.clu_Vanz * 0.45: # keep decel when cut_in, max running time 8sec
