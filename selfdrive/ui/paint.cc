...
static void ui_draw_debug(UIState *s) {
  ...
  if (scene.nDebugUi2) {
    ...
    ui_print(s, ui_viz_rx, ui_viz_ry+480, "←%.1f%","%.1f%→", scene.lateralPlan.lProb*100, scene.lateralPlan.rProb*100);
    ui_print(s, ui_viz_rx, ui_viz_ry+520, "↔%.1f%","%.1fm", scene.lateralPlan.dProb*100, scene.lateralPlan.laneWidth); // High dProb is more related to LaneLine, Low is Laneless
    ...
    if (scene.cal_view) {
      ui_print(s, ui_viz_rx, ui_viz_ry+560, "↕%.1f°", "↔%.1f°", scene.accel_prob[0], scene.accel_prob[1]);
    }
